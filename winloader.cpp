// =====================================================================
//  winloader.cpp - Windows 可执行文件加载器 (NexOS 内核版)
// ---------------------------------------------------------------------
...
static int wl_exec_bat_lines(const uint8_t* data, int size, int start_line, int* last_line) {
    ...
    wl_env_set("COMSPEC", "NexOS-shell");
    wl_env_set("OS", "NexOS");
    ...
    while (pos < size) {
        ...
        if (echo_on) {
            wl_print(l);
            wl_println(" - unknown command (NexOS bat engine)");
        }
    }
...
static int wl_exec_ps1(const uint8_t* data, int size) {
    wl_println("== NexOS PowerShell Engine ==");
...
static int wl_pe_info(const uint8_t* data, int size) {
    ...
    wl_print_num(nsects);
    wl_println("");
    wl_println("  Runtime: NexOS does NOT support Windows PE execution yet.");
    return 0;
}
...
static int wl_com_info(int size) {
    ...
    wl_println("  Runtime: NexOS does NOT support 8086 real-mode COM execution.");
    return 0;
}
