...
.drive_ok:
    ...
    ; kernel.bin can be up to KERNEL_SECTORS*512 bytes.
    ; Each chunk advances the segment by chunk*512/16 = chunk*32.
    ; IMPORTANT: Loop state (remaining/LBA offset/segment) is kept in MEMORY,
    ; NOT in registers. INT 13h is allowed to clobber AX/BX/CX/DX/SI/DI/BP
    ; (AH = status code on return), so keeping the counters in registers made
    ; the load stop early on some SeaBIOS versions (kernel loaded truncated
    ; -> executing missing .text -> triple fault).
    mov byte [dap_kernel + 2], KERNEL_CHUNK     ; sectors per call
    mov word [ld_remain], KERNEL_SECTORS        ; remaining sectors
    mov word [ld_lbaoff], 0                     ; LBA offset (sectors)
    mov word [ld_seg], KERNEL_LOAD_SEG          ; segment (0x1000 -> linear 0x10000)
.load_loop:
    mov ax, [ld_remain]
    or ax, ax
    jz .load_done
...
.chunk_ok:
    ...
    mov ax, [ld_seg]
    mov word [dap_kernel + 6], ax                ; segment
    movzx eax, word [ld_lbaoff]
    add eax, KERNEL_LBA
    mov dword [dap_kernel + 8], eax              ; LBA (32-bit)

    ; Ensure ES=0 for DAP pointer
    xor ax, ax
    mov es, ax

    ; Clear upper 16 bits of 32-bit registers to prevent BIOS handler
    ; from using garbage high bits for addressing (crash on some SeaBIOS).
    and ebx, 0x0000FFFF
    and ecx, 0x0000FFFF
    and edx, 0x0000FFFF
    and esi, 0x0000FFFF
    ...
    ; Update loop state from memory (INT 13h clobbered the registers)
    mov ax, [ld_remain]
    sub ax, KERNEL_CHUNK
    mov [ld_remain], ax
    mov ax, [ld_lbaoff]
    ...
    mov [ld_lbaoff], ax
    mov ax, [ld_seg]
    add ax, KERNEL_CHUNK * 32                    ; segment += chunk*32
    mov [ld_seg], ax
    jmp .load_loop
.load_done:
    ...
    ; Best match tracking: prefer a COMPACT desktop-friendly resolution so the
    ; emulator window does not swallow the whole host screen / taskbar.
    ; Score: 8=1280x720 (preferred), 7=1024x768, 6=800x600, 5=1280x1024,
    ;        4=1366x768, 3=1600x900, 2=1920x1080, 1=640x480 (last resort)
...
.vbe_check_1280_720:
    cmp word [vbe_best_score], 8
    jae .vbe_enum_skip
    mov word [vbe_best_score], 8
    pop cx
    push cx
    mov [vbe_best_mode], cx
    jmp .vbe_enum_loop
...
.vbe_no_match:
    ...
    ; Compact-first order (keep emulator window smaller than the host screen),
    ; only then fall back to the oversized modes.
    mov cx, 0x147                  ; Bochs: 1280x720x32   (preferred)
    call vbe_query_only
    jnc .vbe_settled
    ...
    call vbe_query_only
    jnc .vbe_settled
    ; Oversized modes - last resort only
...
.vbe_settled:
    ...
    cmp ax, 0x004F
    jne .vbe_set_failed
    mov byte [0x500E], 1          ; VBE mode successfully set via INT 10h
...
msg_error_hint: db '  Hint: boot via build/os.img (not os.iso).', 13, 10, 0
...
; =====================================================================
boot_drive:  db 0
; ----- Kernel load loop state (kept in memory because INT 13h clobbers regs) -----
ld_remain:   dw 0            ; remaining sectors to load
ld_lbaoff:   dw 0            ; LBA offset (sectors from kernel start)
ld_seg:      dw 0            ; current load segment
