...
static void nic_send(const uint8_t* data, int len){
    ...
    for (int i = 0; i < 100000; i++){
        ...
        if (ninb(NE_BASE + NE_ISR) & ISR_TXE){
            net_serial("[NIC] TX error\n");
            noutb(NE_BASE + NE_ISR, 0xFF);
            return;
        }
    }
    // Timeout - clear anyway
    net_serial("[NIC] TX timeout\n");
    noutb(NE_BASE + NE_ISR, 0xFF);
}
...
// `src`/`dst` must already be in wire order (i.e. htonl'd), because the sum
// below is accumulated in the "little-endian word" domain: every 16-bit term
// is read from memory natively and the final value is stored back natively,
// which keeps the one's-complement sum byte-order agnostic.  The two scalar
// pseudo-header fields have to join that same domain, so they get swapped
// too -- adding a plain 0x0006 / len (the big-endian reading) silently
// corrupted every TCP checksum and made SLIRP drop our SYN.
static uint16_t tcp_checksum(uint32_t src, uint32_t dst, void* data, int len){
    ...
    sum += (dst >> 16) & 0xFFFF;
    sum += dst & 0xFFFF;
    sum += 0x0600;                              // protocol = TCP (htons(6))
    sum += (uint32_t)htons((uint16_t)len);      // TCP length
    // TCP data
    uint16_t* p = (uint16_t*)data;
    while (len > 1){
        sum += *p++;
...
static bool arp_lookup(uint32_t ip, uint8_t* mac){
    ...
    // A miss is worth reporting: it means the packet is dropped and only an
    // ARP request goes out, which is the classic "silent no-network" symptom.
    net_serial("[ARP] miss "); net_serial_ip(ip); net_serial("\n");
    return false;
}
...
static void tcp_send_raw(uint32_t dst_ip, uint16_t src_port, uint16_t dst_port,
    ...
    // Calculate TCP checksum.  The pseudo-header must use the IP addresses in
    // NETWORK byte order, so convert before passing them in.
    tcp->checksum = tcp_checksum(htonl(our_ip), htonl(dst_ip), seg, 20 + len);
...
static void handle_ip(const uint8_t* data, int len){
    ...
    // Verify destination
    if (ntohl(ip->dst_ip) != our_ip) return;
...
// Per-poll serial tracing is OFF by default.  net_poll() runs on every
// iteration of the shell main loop; the 16550 UART is driven synchronously
// with a busy-wait, so unconditional tracing here costs milliseconds per
// iteration and makes the kernel drop PS/2 keystrokes (the 8042 only buffers
// one scancode).  Enable it explicitly when debugging the network stack.
bool g_net_trace = false;  // set true to log every received frame
...
void net_poll(void){
    ...
    if (len > 0){
        if (g_net_trace){
            char b[16]; int i=0,t=len;
            if(t==0){b[i++]='0';}
            while(t){b[i++]='0'+(t%10);t/=10;}
            b[i]=0;
            for(int j=0,k=i-1;j<k;j++,k--){char c=b[j];b[j]=b[k];b[k]=c;}
            net_serial("[NP] rx len="); net_serial(b); net_serial("\n");
        }
