...
Once kernel64.bin grew past 400 KiB (>=624 sectors) it silently overwrote both
the SFS image at LBA 800 and the start of the ESP, so OVMF could no longer
mount the FAT filesystem and dropped into the EFI Shell.  Keep a generous gap
in front of the ESP whenever these numbers are touched.
...
def main():
    ...
    # Write kernel64.bin at LBA KERNEL64_LBA if provided
    if kernel64_path and os.path.exists(kernel64_path):
        ...
        # Hard failure instead of a silent overwrite: a kernel64 that spills
        # into the SFS area or the ESP produces an image that boots into the
        # EFI Shell with no obvious cause.
        if KERNEL64_LBA + k64_sectors > SFS_LBA:
            print(f"ERROR: kernel64.bin needs {k64_sectors} sectors at LBA "
                  f"{KERNEL64_LBA} but SFS starts at LBA {SFS_LBA} "
                  f"(overlap of {KERNEL64_LBA + k64_sectors - SFS_LBA} sectors)."
                  " Move SFS_LBA/ESP_LBA_START further out.")
            sys.exit(1)
    ...
    # Write SFS image at LBA SFS_LBA if provided
    if sfs_path and os.path.exists(sfs_path):
        ...
        if SFS_LBA + sfs_sectors > ESP_LBA_START:
            print(f"ERROR: SFS needs {sfs_sectors} sectors at LBA {SFS_LBA} "
                  f"but the ESP starts at LBA {ESP_LBA_START}.")
            sys.exit(1)
