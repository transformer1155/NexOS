#!/usr/bin/env python3
"""Headless proof that a REAL 32-bit Windows PE (hello_fileio.exe, cross
compiled with i686-w64-mingw32-gcc) loads and executes through the NexOS
win32_run loader and exercises genuine Win32 APIs:
...
Flow:
  1. Boot BIOS os.img.
  2. Login root/admin at the shell.
  3. `winapp hello_fileio.exe` -> win32_run maps/executes the PE32 image.
  4. `cat hello.txt` -> read back the file the PE wrote.
"""
import os
import socket
import subprocess
...
IMG = "build/os.img"
LOG = "build/serial_win32_fileio.log"
MON_ERR = "build/qemu_win32_fileio.err"
PORT = 4479

BOOT_MARKERS = ("Shell ready", "[MFORMS]", "[K32]", "SFS:")
...
def main():
    ...
    print("\n" + "=" * 66)
    print("Real Win32 PE (hello_fileio.exe) executed on NexOS")
    print("=" * 66)
    ...
    for line in txt.splitlines():
        if "[HELLO_PE]" in line or "Hello from a real Windows" in line:
            print("  " + line.rstrip())
