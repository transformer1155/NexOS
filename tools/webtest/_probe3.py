    ...
    def drain(label, t=3.0):
        ...
        while True:
            ...
            if x is None:
                break
            out.append(x)
        print("[%s] %s" % (label, repr("".join(out)[:600])))
        print()

    # 1) First, observe what kernel sends on connect (login prompt? shell ready?)
    drain("initial kernel output after connect", 3.0)

    # 2) Try sending 'about' directly
    fs("about")
    drain("after 'about'", 2.0)

    # 3) Try 'help'
    fs("help")
    drain("after 'help'", 2.0)
