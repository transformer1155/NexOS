...
#define KERNEL64_LBA     400    // LBA where kernel64.bin is stored on disk
#define KERNEL64_SECTORS 320    // 160KB max (320 * 512 bytes)
...
extern "C" void kmain(){
    ...
    if(g_vbe_active){
        ...
        gui_init();
        if(gui_available()){
            serial_puts("[K-VBE] GUI initialized successfully (Win11 Desktop)\n");
            serial_puts("[K-VBE] Auto-entering GUI desktop...\n");
            gui_enter();
        } else {
            serial_puts("[K-VBE] GUI init failed - showing diagnostic on screen\n");
            g_vbe_active = false;

            // Show diagnostic message on screen via VGA text buffer
            // (works even if VBE framebuffer is broken)
            if (!g_fb_console_mode) {
                // In text mode, write directly to terminal
                term.set_color(make_color(RED, BLACK));
                term.write("\n*** GUI Initialization Failed ***\n");
                term.set_color(make_color(YELLOW, BLACK));
                term.write("VBE info was available but GUI could not initialize.\n");
                term.write("Possible causes:\n");
                term.write("  - Framebuffer address above 4GB\n");
                term.write("  - Unsupported pixel format\n");
                term.write("  - Insufficient memory for backbuffer\n");
                term.set_color(make_color(WHITE, BLACK));
                term.write("\nFalling back to text mode terminal.\n");
                term.write("Type 'help' for available commands.\n\n");
            } else {
                // In framebuffer mode, the fb_console will render the VGA text buffer
                term.set_color(make_color(RED, BLACK));
                term.write("\n*** GUI Initialization Failed ***\n");
                term.set_color(make_color(YELLOW, BLACK));
                term.write("VBE mode is set but GUI init failed.\n");
                term.write("Check serial output for details.\n");
                term.set_color(make_color(WHITE, BLACK));
                term.write("\nFalling back to text mode terminal.\n\n");
            }
        }
    } else {
        // No VBE available - show diagnostic
        term.set_color(make_color(YELLOW, BLACK));
        term.write("\n*** No Graphics Mode Available ***\n");
        term.write("Running in text mode only.\n");
        term.set_color(make_color(WHITE, BLACK));
        term.write("Type 'help' for available commands.\n\n");
    }
