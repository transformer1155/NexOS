...
struct Graphics {
    ...
    void init() {
        ...
        if (info->vbe_ok != 1) {
            initialized = false;
            serial_puts("[GUI] VBE not available\n");
            return;
        }
    ...
    void fill_gradient(int x, int y, int w, int h, Color top, Color bot) {
        for (int row = 0; row < h; row++) {
            ...
            uint8_t r = (uint8_t)(top >> 16) * (1-t) + (uint8_t)(bot >> 16) * t;
            uint8_t g = (uint8_t)(top >> 8) * (1-t) + (uint8_t)(bot >> 8) * t;
            uint8_t b = (uint8_t)(top) * (1-t) + (uint8_t)(bot) * t;
            Color c = ((uint32_t)r << 16) | ((uint32_t)g << 8) | b;
