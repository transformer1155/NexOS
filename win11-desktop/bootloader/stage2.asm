...
KERNEL_SECTORS     equ 360         ; 360 sectors = 180 KiB (room for Win11 GUI + FS + shell)
KERNEL_CHUNK       equ 128         ; sectors per INT 13h call (BIOS-safe limit)
KERNEL_ENTRY       equ 0x10000     ; kernel entry (linked at this address)

start:
    mov [boot_drive], dl           ; boot drive passed from Stage 1

    mov si, msg_hello
    call print_string

    ; ----- 1. Load the kernel in three chunks (128 sectors each) -----
    ; Chunk 1: LBA 33, 128 sectors -> 0x10000
    mov si, dap_kernel
    mov ah, 0x42
    mov dl, [boot_drive]
    int 0x13
    ...
    ; Chunk 2: LBA 33+128=161, 128 sectors -> 0x10000 + 128*512 = 0x20000
    ; Update DAP: sector count, buffer offset, LBA
    mov word [dap_kernel + 2], KERNEL_CHUNK    ; sectors
    mov word [dap_kernel + 4], 0x0000          ; offset (wraps segment)
    mov word [dap_kernel + 6], 0x2000          ; segment = 0x2000 -> linear 0x20000
    mov dword [dap_kernel + 8], KERNEL_LBA + KERNEL_CHUNK  ; LBA
    mov si, dap_kernel
    mov ah, 0x42
    mov dl, [boot_drive]
    int 0x13
    ...
    ; Chunk 3: LBA 33+256=289, 128 sectors -> 0x10000 + 256*512 = 0x30000
    ; Segment 0x3000 -> linear 0x30000
    mov word [dap_kernel + 2], KERNEL_CHUNK    ; sectors
    mov word [dap_kernel + 4], 0x0000          ; offset
    mov word [dap_kernel + 6], 0x3000          ; segment = 0x3000 -> linear 0x30000
    mov dword [dap_kernel + 8], KERNEL_LBA + KERNEL_CHUNK * 2  ; LBA
    mov si, dap_kernel
    mov ah, 0x42
    mov dl, [boot_drive]
    int 0x13
...
.vbe_settled:
    ...
    ; This switches the display to graphics mode with linear framebuffer
    mov bx, cx
    ...
    ; Mode set successfully - display is now in VBE graphics mode
    mov byte [0x500E], 1          ; vbe_mode_set = 1

    ; Serial: 'Q' = VBE mode set successfully
    mov dx, 0x3F8
    mov al, 'Q'
    out dx, al
...
.vbe_set_failed:
    ; Mode set failed - VBE info is available but mode was NOT set
    ; Kernel will try Bochs VBE ports (emulator) or skip GUI
    mov byte [0x500E], 0          ; vbe_mode_set = 0

    ; Serial: 'q' = VBE info stored but mode NOT set
    mov dx, 0x3F8
    mov al, 'q'
    out dx, al
